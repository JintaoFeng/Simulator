	...
	//		        Machine.fineStage.arrAxis[FDZ].axisCMD = C_Aixs_RunFor;
		...
		case EXPOSet:
			if(Machine.coarseStage.stageStatus == S_Stage_Traj_Expo)
			{
                ...
                Machine.coarseStage.stageCMD = (StageCMD)C_Nothing;
				Coarse_Count = 0;
			}
...
/*			if(DirParam[3] == 1)
	...
	//		break;
		case Coarse_Both_MOVE:
            Machine.coarseStage.stageStatus = S_Stage_Traj_Expo;
			/*if(DirParam[0] > 0)
			{
              Machine.coarseStage.arrAxis[0].cTrig=1;
			Machine.coarseStage.arrAxis[1].cTrig=1;			
...
//			printf("Acc:%lf",rxData.dParamData[0]);
	...
	//		printf("posOffset:%lf",Machine.coarseStage.arrAxis[rxData.axis].posOffset);
		...
		case SET_CONNECT_TYPE:
            Machine.connectType = (ConnectType)rxData.iReserved[0];
			break;
		case CLEAR:
			Machine.coarseStage.arrAxis[0].dActPos=0;
			Machine.coarseStage.arrAxis[0].dSetPoint=0;
			Machine.coarseStage.arrAxis[0].dSolveMethodOne=0;
			memset(Machine.coarseStage.arrAxis[0].controlObject.pUout,0,sizeof(double)*Machine.coarseStage.arrAxis[0].controlObject.order);

			memset(Machine.coarseStage.arrAxis[0].controlObject.pIn,0,sizeof(double)*Machine.coarseStage.arrAxis[0].controlObject.order);
		default:
		break;
	}
	*pCMD=0;
