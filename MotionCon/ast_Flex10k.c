...
static const char RCSid[]="PLD - FLEX10K Loader: $Id: flex10k.c,v 1.5 1999/07/21 17:43:31 Franke Exp $";
...
/*        int slot, */
        ...
        UINT32 flags,
        void *arg,
        void (*msec_delay)(void *arg, UINT32 msec),
        UINT16 nonpld_bits,
        UINT8 data, 
        UINT8 dclk, 
        UINT8 config, 
        ...
        UINT8 cfgdone)
{
    ...
    +--------------------------------------*/
        ac.ma           = vme_addr/*VMIC_A24_BASE + vme_addr + slot*0x200*/ + M72_FPGA_REGISTER;
	ac.arg		= arg;
	...
	ac.config  = 1 << config; 
	ac.status  = 1 << status;
	ac.cfgdone = 1 << cfgdone;

	/* initial control register state */
	ac.ctrl_bits = nonpld_bits & ~(ac.config | ac.status | ac.cfgdone);		 
